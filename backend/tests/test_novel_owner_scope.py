...
from app.domain.entities.novel_runtime import NovelReadingProgress
...
@pytest.mark.asyncio
async def test_legacy_novel_data_can_be_claimed_by_the_authenticated_owner(owner_scoped_repo):
    legacy_book = await owner_scoped_repo.save_book(
        "legacy", NovelBook(book_url="https://legacy.test/book", book_name="旧书")
    )
    chapter = NovelChapter(book_id=legacy_book.id, canonical_full="C1", canonical_num=1, raw_text="旧正文")
    await owner_scoped_repo.save_chapter("legacy", chapter)
    await owner_scoped_repo.save_reading_progress(
        NovelReadingProgress(owner_scope="legacy", book_id=legacy_book.id, chapter_id=chapter.id, percent=0.4)
    )

    result = await owner_scoped_repo.claim_legacy_scope("user:1")

    assert result["books"] == 1
    assert await owner_scoped_repo.get_book_by_id("legacy", legacy_book.id) is None
    claimed = await owner_scoped_repo.get_book_by_id("user:1", legacy_book.id)
    assert claimed is not None
    progress = await owner_scoped_repo.get_reading_progress("user:1", legacy_book.id)
    assert progress is not None
    assert progress.percent == 0.4
...
def test_linked_api_key_uses_the_same_novel_owner_scope_as_its_user():
    from app.interfaces.http.deps import ApiKeyIdentity, owner_scope_for

    identity = ApiKeyIdentity(7, "reader", {"novel.manage"}, owner_user_id=3)

    assert owner_scope_for(identity) == "user:3"
