...
@pytest.mark.asyncio
async def test_runtime_service_requires_review_resolution_before_publishing_candidate(tmp_path, monkeypatch):
    ...
    published = await service.resolve_review(candidate["source_version_id"], reviewer_id="reviewer-1", action="publish")
    ...
    assert published["review_action"] == "publish"
    assert published["reviewed_by"] == "reviewer-1"
