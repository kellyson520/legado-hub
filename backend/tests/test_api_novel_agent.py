...
class Ingestion:
    ...
    async def import_source(self, owner_scope, source_id, book_url, book_name, author):
        assert (owner_scope, source_id, book_url, book_name, author) == ("user:1", 3, "https://book.test/1", "测试书", "作者")
        return type("ImportResult", (), {"book_id": 8, "duplicate": False, "status": "queued", "task_id": "task-2", "error_code": None})()

    async def import_url(self, owner_scope, url, title, author):
        assert (owner_scope, url, title, author) == ("user:1", "https://book.test/2", "网络书", "作者")
        return type("ImportResult", (), {"book_id": 9, "duplicate": False, "status": "queued", "task_id": "task-3", "error_code": None})()
...
def test_upload_is_scoped_and_legacy_chat_still_works(monkeypatch):
    ...
    from app.interfaces.http import novel_agent_compat as novel_agent_api
    ...
    monkeypatch.setattr(novel_http, "get_novel_agent_app_service", lambda: _async_value(Agent()))
    ...
    source = client.post(
        "/api/v1/novel-agent/import/source",
        headers=owner,
        json={"source_id": 3, "book_url": "https://book.test/1", "book_name": "测试书", "author": "作者"},
    )
    assert source.status_code == 200
    assert source.json()["data"]["book_id"] == 8
    url = client.post(
        "/api/v1/novel-agent/import/url",
        headers=owner,
        json={"url": "https://book.test/2", "title": "网络书", "author": "作者"},
    )
    assert url.status_code == 200
    assert url.json()["data"]["book_id"] == 9
...
def test_legacy_novel_data_can_be_claimed_by_the_current_user(monkeypatch):
    from app.interfaces.http import novel as novel_http

    class ClaimingRepo:
        async def claim_legacy_scope(self, owner_scope):
            assert owner_scope == "user:1"
            return {"books": 2}

    class ClaimingRuntime:
        def claim_legacy_scope(self, owner_scope):
            assert owner_scope == "user:1"
            return {"ingestions": 2, "tasks": 1}

    monkeypatch.setattr(novel_http, "get_scoped_novel_repository", lambda: _async_value(ClaimingRepo()))
    monkeypatch.setattr(novel_http, "build_novel_runtime_repository", lambda: ClaimingRuntime())

    from app.main import app

    response = TestClient(app).post(
        "/api/novel/legacy/claim",
        headers=_headers(1, ["novel.manage"]),
    )
...
def test_reader_conversation_request_keeps_the_initial_chapter_context():
    from app.interfaces.http.novel import ConversationRequest

    payload = ConversationRequest(book_id=7, chapter_id=2, entrypoint="reader")
