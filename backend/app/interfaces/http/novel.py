...
from app.interfaces.api.v1.novel import get_novel_repository
...
async def get_scoped_novel_repository():
    return await get_novel_repository()
...
@router.get("/books")
async def list_books(identity=Depends(require_principal_permission(Permission.NOVEL_MANAGE))):
    owner_scope = owner_scope_for(identity)
    repo = await get_scoped_novel_repository()
    books = await repo.list_books(owner_scope, limit=100, offset=0)
    output = []
    for book in books:
        item = _serialize(book)
        progress = await repo.get_reading_progress(owner_scope, book.id)
        item["progress"] = _serialize(progress)
        output.append(item)
    return {"success": True, "code": "OK", "message": "novels listed", "data": output, "meta": {"total": len(output)}, "trace_id": None}
...
async def create_novel_conversation(payload: ConversationRequest, identity=Depends(require_principal_permission(Permission.AI_RUN))):
    ...
    data = await service.create_conversation(owner_scope_for(identity), payload.title, book_id=payload.book_id, entrypoint=payload.entrypoint, model_ref=payload.model)
