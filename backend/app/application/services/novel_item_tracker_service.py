...
class NovelItemTrackerService:
    ...
    def __init__(self):
        ...
        self._p_transfer = re.compile(
            r"(?:将|把|以)(?P<item>[\u4e00-\u9fff]{2,8}?)(?P<trans_verb>赐予|赠予|授予|赠与|授与|交付|献给|送与|留给|赏赐|托付|遗下|奉上|递给|交给|送给)(?P<target>[\u4e00-\u9fff]{2,4})"
        )
        ...
        for chapter in chapters:
            ...
            for sentence in sentences:
                ...
                for tm in self._p_transfer.finditer(sentence):
                    t_item = tm.group("item").strip()
                    t_target = tm.group("target").strip()
                    if t_item and any(s in t_item for cat in self._CATEGORY_SUFFIXES.values() for s in cat):
                ...
                for qm in self._p_quant.finditer(sentence):
                    raw_item = qm.group("item").strip()
                    if raw_item and any(s in raw_item for cat in self._CATEGORY_SUFFIXES.values() for s in cat):
