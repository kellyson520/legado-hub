...
class LegadoBookSourceFetcher:
    ...
    async def search(
        ...
        if search_url_tmpl.strip().startswith("@js:"):
            ...
            response_payload = output.value.get("response") if isinstance(output.value, dict) else None
        ...
        book_list = self._extract_list(resp_data, book_list_rule, response_is_html, base_url)
        if not book_list:
            return []
        ...
        for item in book_list:
            ...
            for field, rule in field_map.items():
                if not rule:
                    continue
                value = self._extract_field(item, rule, base_url, response_is_html, field)
                book_info[field] = value

            if book_info.get("bookUrl") and not book_info["bookUrl"].startswith("http"):
                book_info["bookUrl"] = UrlUtils.resolve_relative(book_info["bookUrl"], base_url)
            if book_info.get("coverUrl") and not book_info["coverUrl"].startswith("http"):
                book_info["coverUrl"] = UrlUtils.resolve_relative(book_info["coverUrl"], base_url)
    ...
    async def get_toc(
        ...
        if toc_url_rule:
            try:
                detail_resp = await self._http.get(book_url, headers=headers)
            except Exception as e:
            ...
            if toc_url and not toc_url.startswith("http"):
                toc_url = UrlUtils.resolve_relative(toc_url, base_url)
        ...
        try:
            resp = await self._http.get(final_url, headers=headers)
        except Exception as e:
        ...
        chapter_list = self._extract_list(resp_data, chapter_list_rule, resp.is_html, final_url)
        if not chapter_list:
            return []
        ...
        for idx, item in enumerate(chapter_list):
            title = self._extract_field(item, name_rule, final_url, resp.is_html, "name")
            url = self._extract_field(item, url_rule, final_url, resp.is_html, "url")
            if url and not url.startswith("http"):
                url = UrlUtils.resolve_relative(url, final_url)
            if title and url:
                chapters.append(
                    {
                        "title": title,
    ...
    async def get_content(
        ...
        headers = UrlUtils.parse_headers(source.get("header", ""))
        try:
            resp = await self._http.get(chapter_url, headers=headers)
        except Exception as e:
        ...
        content = self._extract_field(resp_data, content_rule, base_url, resp.is_html, "content")
        title = self._extract_field(resp_data, title_rule, base_url, resp.is_html, "name")
        next_url = self._extract_field(resp_data, next_url_rule, chapter_url, resp.is_html, "url")

        if next_url and not next_url.startswith("http"):
            next_url = UrlUtils.resolve_relative(next_url, chapter_url)
    ...
    async def get_book_info(
        ...
        headers = UrlUtils.parse_headers(source.get("header", ""))
        try:
            resp = await self._http.get(book_url, headers=headers)
        except Exception:
            return {}
    ...
    def _extract_field(
        ...
        if isinstance(data, dict) and "{{" in rule and "}}" in rule:
