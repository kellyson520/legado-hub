      ...
      {navigationGroups.map((group) => {
        ...
        return (
          <section key={group.label} className="space-y-1">
            {!collapsed ? <p className="px-2 pb-1 text-[11px] font-semibold text-muted-foreground">{group.label}</p> : null}
...
function ThemeControl() {
  ...
  const options: Array<{ mode: ThemeMode; label: string; icon: typeof Sun }> = [
    ...
    { mode: 'system', label: '跟随系统主题', icon: Monitor },
  ]
  return (
    <div aria-label="主题" className="hidden items-center rounded-md border border-border bg-muted p-0.5 sm:flex">
    ...
    <div className="min-h-screen bg-background text-foreground">
      <aside className={cn('fixed inset-y-0 left-0 z-30 hidden border-r border-border bg-card lg:flex lg:flex-col', collapsed ? 'w-[72px]' : 'w-[248px]')}>
        <div className={cn('flex h-16 items-center border-b border-border px-4', collapsed ? 'justify-center' : 'justify-between')}>
          ...
          {!collapsed ? <Button variant="ghost" size="icon" title="收起侧边栏" aria-label="收起侧边栏" onClick={() => setCollapsed(true)}><PanelLeftClose className="h-4 w-4" /></Button> : null}
        </div>
        {collapsed ? <Button variant="ghost" size="icon" title="展开侧边栏" aria-label="展开侧边栏" className="mx-auto mt-3" onClick={() => setCollapsed(false)}><PanelLeftOpen className="h-4 w-4" /></Button> : null}
        <Navigation collapsed={collapsed} />
        {!collapsed ? <p className="border-t border-border px-5 py-3 text-[11px] text-muted-foreground">LEGADO CONTROL / 2.1</p> : null}
      </aside>

      {mobileOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="导航菜单">
          <button aria-label="关闭导航遮罩" className="absolute inset-0 cursor-default bg-black/45" onClick={() => setMobileOpen(false)} />
          <aside className="relative flex h-full w-[280px] flex-col border-r border-border bg-card shadow-2xl">
            <div className="flex h-16 items-center justify-between border-b border-border px-4">
              ...
              <Button variant="ghost" size="icon" aria-label="关闭导航" onClick={() => setMobileOpen(false)}><X className="h-4 w-4" /></Button>
            </div>
      ...
      <div className={cn('min-h-screen transition-[padding] lg:pl-[248px]', collapsed && 'lg:pl-[72px]')}>
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border bg-background/95 px-4 backdrop-blur lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            <Button variant="ghost" size="icon" className="lg:hidden" aria-label="打开导航" onClick={() => setMobileOpen(true)}><Menu className="h-5 w-5" /></Button>
            <div className="min-w-0">
              <p className="text-[11px] font-medium text-muted-foreground">控制台 / {current?.label ?? '概览'}</p>
              <p className="truncate text-sm font-semibold text-foreground">{current?.label ?? 'Legado Hub'}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="hidden items-center gap-1.5 text-xs text-muted-foreground md:flex"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />API Online</span>
            <ThemeControl />
            <div className="ml-1 flex items-center gap-2 border-l border-border pl-3">
              <span className="hidden text-right text-xs leading-4 sm:block"><span className="block font-medium text-foreground">{session?.user.username ?? '账户'}</span><span className="text-muted-foreground">已登录</span></span>
              <Button variant="ghost" size="icon" aria-label="退出登录" title="退出登录" onClick={() => void logout()}><LogOut className="h-4 w-4" /></Button>
            </div>
          </div>
        </header>
