...
export async function uploadNovel(file: File) {
  ...
  return apiClient.post<NovelImportResult>('/v1/novel-agent/import/upload', form, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
}

export function importNovelFromSource(result: Pick<NovelSearchResult, 'sourceId' | 'bookUrl' | 'name' | 'author'>) {
  return apiClient.post<NovelImportResult>('/v1/novel-agent/import/source', {
...
export function importNovelFromUrl(url: string, title = '', author = '') {
  return apiClient.post<NovelImportResult>('/v1/novel-agent/import/url', { url, title, author })
}
...
export async function listNovelTasks() {
  ...
  return {
    ...
    data: response.data.map((book) => ({
      ...
      provider: book.source_name ?? 'n/a',
      pipeline: 'novel-index',
    })),
  } satisfies ApiEnvelope<NovelTaskRow[]>
}
