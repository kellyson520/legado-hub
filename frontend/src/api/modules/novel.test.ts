...
test('novel import helpers use the canonical bookshelf import endpoints', async () => {
  postMock.mockClear()
  postMock
    .mockResolvedValueOnce({ success: true, code: 'OK', message: 'queued', data: {}, meta: {}, trace_id: null })
    .mockResolvedValueOnce({ success: true, code: 'OK', message: 'queued', data: {}, meta: {}, trace_id: null })
    .mockResolvedValueOnce({ success: true, code: 'OK', message: 'queued', data: {}, meta: {}, trace_id: null })

  await uploadNovel(new File(['正文'], 'book.txt', { type: 'text/plain' }))
  await importNovelFromSource({ sourceId: 3, bookUrl: 'https://book.test/1', name: '测试书', author: '作者' })
  await importNovelFromUrl('https://book.test/1', '测试书', '作者')

  expect(postMock).toHaveBeenNthCalledWith(1, '/novel/books/import/upload', expect.any(FormData), {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
  expect(postMock).toHaveBeenNthCalledWith(2, '/novel/books/import/source', {
    source_id: 3,
    book_url: 'https://book.test/1',
    book_name: '测试书',
    author: '作者',
  })
  expect(postMock).toHaveBeenNthCalledWith(3, '/novel/books/import/url', {
