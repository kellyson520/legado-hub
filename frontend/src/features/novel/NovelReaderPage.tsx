...
export function NovelReaderPage() {
  ...
  useEffect(() => {
    ...
    async function loadReader() {
      ...
      try {
        ...
        if (progressResponse.data?.line_height) setLineHeight(progressResponse.data.line_height)
        if (progressResponse.data?.content_width === 'compact' || progressResponse.data?.content_width === 'comfortable') setContentWidth(progressResponse.data.content_width)
        setError('')
      } catch {
  ...
  useEffect(() => {
    if (!chapter || !progress || progress.chapter_id !== chapterId) return
    const key = `${bookId}:${chapterId}`
    if (restoredPositionRef.current === key) return
    restoredPositionRef.current = key
    const timer = window.setTimeout(() => {
      const root = document.documentElement
      const scrollHeight = Math.max(root.scrollHeight, document.body?.scrollHeight ?? 0)
      const maxScroll = Math.max(scrollHeight - Math.max(window.innerHeight, 1), 0)
      window.scrollTo({ top: Math.round(progress.percent * maxScroll), behavior: 'auto' })
    }, 0)
    return () => window.clearTimeout(timer)
  }, [bookId, chapterId, chapter, progress])

  const persistProgress = (next: { theme?: ReaderTheme; fontSize?: number; background?: string; lineHeight?: number; contentWidth?: ContentWidth; offsetChars?: number; percent?: number } = {}) => {
    ...
    void saveNovelProgress(bookId, {
      chapterId,
      offsetChars: next.offsetChars ?? progress?.offset_chars ?? 0,
      percent: next.percent ?? progress?.percent ?? 0,
      preferences,
    })
  }
  ...
  useEffect(() => {
    ...
    const scheduleProgressSave = () => {
      ...
      timer = window.setTimeout(() => {
        const root = document.documentElement
        const scrollHeight = Math.max(root.scrollHeight, document.body?.scrollHeight ?? 0)
        const maxScroll = Math.max(scrollHeight - Math.max(window.innerHeight, 1), 1)
        const percent = Math.max(0, Math.min(1, (window.scrollY || root.scrollTop || 0) / maxScroll))
        const offsetChars = Math.round(percent * (chapter.raw_text?.length ?? 0))
        setProgress((current) => current ? { ...current, offset_chars: offsetChars, percent } : current)
        persistProgress({ offsetChars, percent })
      }, 200)
    }
  ...
  }, [bookId, chapterId, chapter, theme, background, fontSize, lineHeight, contentWidth])
