...
export function ApiKeysPage() {
  const [keys, setKeys] = useState<ApiKeyRow[]>([]); const [name, setName] = useState(''); const [permissions, setPermissions] = useState<string[]>(['read.work']); const [revealed, setRevealed] = useState(''); const [error, setError] = useState('')
  const load = async () => { try { setKeys((await listApiKeys()).data); setError('') } catch { setError('无法加载 API Key') } }
  useEffect(() => { void load() }, [])
  const create = async () => { if (!name.trim()) return; try { const result = await createApiKey({ name: name.trim(), permissions }); setRevealed(result.data.raw_key || ''); setName(''); await load() } catch { setError('创建 API Key 失败') } }
